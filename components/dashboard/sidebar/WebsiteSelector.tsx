...
export function SidebarWebsiteSelector() {
  ...
  return (
    <div className="p-2 flex items-center justify-between rounded-xl transition-colors cursor-default">
      <div className="flex items-center gap-2 w-full">
        <div className="flex items-center gap-3.5 grow">
          <div className="rounded-full bg-stone-300 w-[18px] h-[18px]" />
          <p className="text-stone-500 font-medium text-sm truncate">
            {websiteName}
          </p>
        </div>
        <Link
          href="/dashboard/settings"
          className="cursor-pointer box-border shrink-0 flex items-center justify-center font-semibold font-mono uppercase border transition-all ease-in duration-75 whitespace-nowrap text-center select-none disabled:shadow-none disabled:opacity-50 disabled:cursor-not-allowed gap-x-1 active:shadow-none active:scale-95 text-sm leading-5 rounded-xl w-8 py-1.5 h-8 border-transparent hover:bg-stone-100 disabled:bg-stone-100"
          title="Settings"
        >
          <svg
