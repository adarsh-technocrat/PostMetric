...
export function SidebarWebsiteSelector() {
  ...
  const currentWebsite = websites.find((w) => w._id === currentWebsiteId);

  const websiteName =
    currentWebsite?.name || currentWebsite?.domain || "Select website";

  const settingsHref = currentWebsiteId
    ? `/dashboard/${currentWebsiteId}/settings`
    : "/dashboard/settings";
    ...
    <div className="flex items-center gap-1 w-full min-w-0">
      <DropdownMenu>
          ...
          >
            {currentWebsite ? (
              <DomainLogo
                ...
                className="rounded-full w-[18px] h-[18px] shrink-0"
              />
            ) : (
              <div className="rounded-full bg-stone-300 w-[18px] h-[18px] shrink-0" />
            )}
            <span className="text-stone-600 font-medium text-sm truncate">
              {websiteName}
            </span>
        ...
        <DropdownMenuContent
          align="start"
          sideOffset={4}
          className="w-[var(--radix-dropdown-menu-trigger-width)] min-w-56 max-h-[min(70vh,400px)] overflow-y-auto"
        >
          {websites.map((site) => (
      ...
      <Link
        href={settingsHref}
        className="cursor-pointer shrink-0 flex items-center justify-center w-8 h-8 rounded-xl border border-transparent hover:bg-stone-100 hover:border-stone-200 transition-colors"
        title={currentWebsiteId ? "Website settings" : "Account settings"}
      >
        <svg
