...
(function() {
  ...
  function getVisitorId() {
    ...
    if (vid) {
      setCookie('postmetric_visitor_id', vid, 365);
      // Clean URL
      try {
    ...
    // Check cookie
    vid = getCookie('postmetric_visitor_id');
  ...
  function getSessionId() {
    ...
    if (sid) {
      setCookie('postmetric_session_id', sid, 1/48); // 30 minutes
      return sid;
    }

    // Check cookie
    sid = getCookie('postmetric_session_id');
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          ...
          setCookie('postmetric_session_id', getSessionId(), 1/48);
        } else {
